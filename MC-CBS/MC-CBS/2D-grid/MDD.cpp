...
bool MDD::buildMDD(const std::vector < std::list<std::pair<int, int> > >& constraints, int current_cost, int lookahead, const SingleAgentICBS & solver)
{
...
bool MDD::updateMDD(const std::tuple<int, int, int> &constraint, int num_col)
{
	int loc1 = std::get<0>(constraint), loc2 = std::get<1>(constraint), t = std::get<2>(constraint);
