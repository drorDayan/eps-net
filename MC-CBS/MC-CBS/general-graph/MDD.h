...
{
public:
	MDDNode(const vertex_t& vertex, MDDNode* parent): vertex(vertex)
	{
		if(parent == NULL)
			level = 0;
		else
	...
	{
		return (this->vertex == node.vertex) && (this->level == node.level);
	}
...
public:
	...
	bool buildMDD(const searchGraph_t& G, const std::vector < std::list< std::pair<vertex_t, vertex_t> > >& constraints, 
		int current_cost, int lookahead, const SingleAgentECBS & solver);
	bool updateMDD(const std::tuple<vertex_t, vertex_t, int> &constraint, int num_col);
	MDDNode* find(const vertex_t& V, int level);
